import pygame
from TowerClimb.core.placeholder.Camera import *
from TowerClimb.core.AssetManager import *
from TowerClimb.core.placeholder.GameObject import *
...
class GameHandle:
    ...
    def __init__(self, missing_image_file_path, window_width: int = 800, window_height: int = 600):
        ...
        self.physics_update_frequency_ticks = 5  # lower value needs more performance #TODO test for good value
        self.gravity = 0.001  # 9.81 is too high
        # Create a camera
    ...
    # ---------------------------
    # Unkommentierter, nicht dokumentierter Code - NUR TESTS !!!
                        ...
                        if r_component.render_layer == current_layer:
                            ...
                            if self.__overlapping_split_screen(game_object.get_x(), image_width, 0, camera):
                                if image_width - (image_width - middle_line) - game_object.get_x() <= 0:  # Nichts mehr auf sichtbarer Seite übrig
                                    continue
                                #image_new = pygame.Surface((game_object.get_x() + image_width - middle_line, image_height))
    ...
    def __clear_screen_split(self, side):
        ...
        surf.fill((0, 0, 0))
        if side == 0:
            #pygame.draw.rect(pygame.display.get_surface(), self.clear_color, pygame.Rect(0 - self.camera.get_x(), 0 - self.camera.get_y(), int(self.window_width/2), self.widow_height), 1)
    ...
    # Unkommentierter, undokumentierter Code - NUR TESTS !!! - ENDE
    # ---------------------------
